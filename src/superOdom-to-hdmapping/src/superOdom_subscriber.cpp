...
int main(int argc, char **argv)
{
    ...
    while (bag.has_next())
    {
        ...
        if (msg->topic_name == "/registered_scan") {
            RCLCPP_INFO(rclcpp::get_logger("SuperOdomScan"), "Received message on topic: /registered_scan");
            ...
            if (!cloud_msg || cloud_msg->data.empty()) {
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("SuperOdomScan"), "Empty PointCloud2 message on /registered_scan");
                continue;
            }
    
            size_t num_points = cloud_msg->width * cloud_msg->height;
    
            RCLCPP_INFO(rclcpp::get_logger("SuperOdomScan"), "Processing %zu points from /registered_scan", num_points);
            ...
            RCLCPP_INFO(rclcpp::get_logger("SuperOdomScan"), "Added %zu points (total: %zu)", num_points, points_global.size());
        }
        
        if (msg->topic_name == "/state_estimation") {
            RCLCPP_INFO(rclcpp::get_logger("SuperOdomOdometry"), "Received message on topic: /state_estimation");
            ...
            if (!odom_msg) {
                RCLCPP_ERROR(rclcpp::get_logger("SuperOdomOdometry"), "Odometry message deserialization error!");
                return 1;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("SuperOdomOdometry"), "Added position to trajectory: x=%.3f, y=%.3f, z=%.3f", x, y, z);
        }
    }
